...
Game_Map.prototype.setFogOfWarState = function(x, y, state) {
    ...
    if (x >= 0 && y >= 0 && x < this.width() && y < this.height()) {
        ...
        if (oldState !== state) {
            // Initialize active transitions tracker if needed
...
Game_Map.prototype.updateTransitionTimers = function() {
    ...
    for (const i of this._activeTransitions) {
        if (i >= size) {
...
Game_Map.prototype.castRay = function(startX, startY, angle, maxDistance) {
    ...
    for (let step = 0; step < maxSteps; step++) {
        ...
        // Apply edge feathering only near max distance
        if (distance > maxDistance - 1) {
            this.applyEdgeFeathering(tileX, tileY);
        }
        
        // Check for vision blocking
...
Game_Map.prototype.isVisionBlocking = function(x, y) {
    ...
    if (cached === undefined) {
        // Cache terrain tag check
        const terrainBlocks = this.terrainTag(x, y) === 4;
...
Game_Map.prototype.isVisionBlockingEvent = function(event) {
    ...
    try {
        ...
        // Check for events named "door_i" or any name from VISION_BLOCKING_EVENT_NAMES
        return eventName === "door_i" || VISION_BLOCKING_EVENT_NAMES.some(blocking => eventName.includes(blocking));
    } catch (error) {
